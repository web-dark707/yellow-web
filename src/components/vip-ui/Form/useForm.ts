        ...
        return new Promise((resolve) => {
            nextTick(() => {
                ...
                const errors = fields.reduce(
                    (pre: Record<string, ReactNode[]>, name) => {
                        const theField = this._fieldsList?.[name];
                        if (theField) {
                            pre[name] = theField._errors;
                        }
                        return pre;
                    },
                    {},
                ...
                // 返回表单的错误状态
                const hasError = Object.values(errors).some((item) => {
                    return (
                        item.length > 0 ||
                        fields.length > Object.values(this._formData).length
                    );
                });
